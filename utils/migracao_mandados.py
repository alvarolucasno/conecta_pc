...
with mysql.connector.connect(**config) as cnx:
    with cnx.cursor(dictionary=True) as cursor:
        select_query = "SELECT * FROM base_inteligencia.detalhamento_pessoa"
        cursor.execute(select_query)
        rows = cursor.fetchall()

        for row in rows:
            # Printando o registro obtido
            print("Registro obtido:", row)

            # Verificar se caminho_foto é nulo ou vazio
            if row['caminho_foto'] is not None and row['caminho_foto'] != '':
                ...
                print("Aviso: Caminho de foto nulo para o ID de pessoa:", row['id_pessoa'])
